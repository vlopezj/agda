...
wakeConstraintsTCM :: (ProblemConstraint-> TCM WakeUp) -> TCM ()
wakeConstraintsTCM wake = do
  c <- useR stSleepingConstraints
  (wakeup, sleepin) <- partitionEithers <$> mapM checkWakeUp c
...
guardConstraint c blocker =   -- Guarded constraints gets unblocked when the blocking problem is solved
  ifNoConstraints_ blocker (solveConstraint c) (addConstraint neverUnblock . Guarded c)
...
wakeConstraints' :: (ProblemConstraint -> TCM WakeUp) -> TCM ()
...
wakeupConstraints x = do
  wakeConstraints' (pure . wakeIfBlockedOnMeta x . constraintUnblocker)
  solveAwakeConstraints
...
wakeupConstraints_ = do
  wakeConstraints' (return . const WakeUp)
  solveAwakeConstraints
...
solveConstraint_ (ValueCmpHet cmp tel a u v) = compareAsHet cmp tel a u v
...
solveConstraint_ (ElimCmpHet ctx cmp fs a u v)   = __IMPOSSIBLE__
solveConstraint_ (TelCmp a b cmp tela telb) = compareTel a b cmp tela telb
solveConstraint_ (TelCmpHet ctx a b cmp tela telb) = compareTelHet ctx a b cmp tela telb
...
solveConstraint_ c0@(Guarded c pid)         = do
  ifM (isProblemSolved pid)
    ...
    {-else-} $ do
      ...
      addConstraint neverUnblock c0 -- Gets unblocked by solving the problem
